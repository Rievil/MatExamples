...
for i=[1 3 5 7]
    ...
    Kalor(n).Name=sprintf('Hex %d',n);
    Kalor(n).Time=eval(['TMP' num2str(n) '(:,1)']);
    Kalor(n).Value=eval(['TMP' num2str(n) '(:,2)']);
    plot(Kalor(n).Time,Kalor(n).Value);
end
    xlim([Kalor(1).Time(1) Kalor(1).Time(2800)]);
    %ylim([0 180]);
...
export=struct();
barva={[0/255 103/255 188/255],[255/255 198/255 30/255]};
...
plCus(9)=plot(xq,[0 620],':','LineWidth',1.8,'Color','k');

str=num2str(round(datenum(xq(1)),2));
tx(9)=text(xq(1)-0.2,630,str,'FontSize',18);
...
video = VideoWriter(['D:\Data\Disk Google\Škola\Doktorské Studium\Mìøení\FCH\VideaFCHHex\Alkal_48fps_v2b.mp4']); %create the video object
video.FrameRate=48;
video.Quality=80;
